...
public function addstudentrollback($mysqli) {
	...
	if(isset($_POST['standard']))		
		...
		$qry = "INSERT INTO studentrollbacktwo(studentname, rollnumber, checks, finalresult,status) VALUES(
			'".strip_tags($studentname)."','".strip_tags($rollnumber)."',
			'".strip_tags($checks)."','".strip_tags($finalresult)."','".strip_tags($status)."'
			)";
			$res =$mysqli->query($qry) or die("Error in Query".$mysqli->error);
			$id = 0;
			$id = $mysqli->insert_id;
...
$qry = 'UPDATE  studentrollback  SET 
standard="'.strip_tags($standard).'" ,section="'.strip_tags($section).'" 

WHERE sturollid ="'.mysqli_real_escape_string($mysqli,$id).'"   ';  


$res =$mysqli->query($qry)or die("Error in in update Query!.".$mysqli->error); 	
...
$qry = 'UPDATE studentrollbacktwo SET
	
studentname="'.strip_tags($studentname).'",rollnumber="'.strip_tags($rollnumber).'",
checks="'.strip_tags($checks).'",finalresult="'.strip_tags($finalresult).'",
status="'.strip_tags($status).'"
WHERE forgid ="'.mysqli_real_escape_string($mysqli,$id).'"   ';  
}
...
{
	$date  = date('Y-m-d'); 
	// $qry = 'UPDATE  studentrollback  WHERE sturollid ="'.mysqli_real_escape_string($mysqli,$del).'"'; 
	// $res =$mysqli->query($qry)or die("Error in delete query".$mysqli->error);	

	$qry = 'UPDATE  studentrollbacktwo  SET status="1"  WHERE stuid ="'.mysqli_real_escape_string($mysqli,$del).'"'; 
	$res =$mysqli->query($qry)or die("Error in delete query".$mysqli->error);
} 
...
public function getstudentrollback($mysqli,$idupd) {
	$qry = "SELECT * FROM studentrollback WHERE sturollid='".mysqli_real_escape_string($mysqli,$idupd)."'"; 
			$res =$mysqli->query($qry)or die("Error in Get All Records".$mysqli->error);
			$detailrecords = array();
			if ($mysqli->affected_rows>0)
			{
				$row = $res->fetch_object();	
			    $detailrecords['id']                		 = $row->sturollid; 
			...
			$qry = "SELECT * FROM studentrollbacktwo WHERE stuid='".mysqli_real_escape_string($mysqli,$idupd)."'"; 
			$res =$mysqli->query($qry)or die("Error in Get All Records".$mysqli->error);
			$detailrecords = array();
			if ($mysqli->affected_rows>0)
			{
				$row = $res->fetch_object();	
			    $detailrecords['id']                     = $row->stuid; 
				$detailrecords['studentname']            = strip_tags($row->studentname);
				$detailrecords['rollnumber']             = strip_tags($row->rollnumber);
